...
- **Docker** — под базу и под само приложение.
...
5. Создать OAuth-клиент типа **Web application** и вписать ему redirect URI — ровно тот,
   что попадёт в `GOOGLE_REDIRECT_URI`, до символа. Порт здесь тот, по которому вы
   открываете приложение: `APP_PORT` при обычном запуске (по умолчанию 8477) или 3000 при
   `pnpm dev`. Адресов можно добавить несколько, так что впишите оба и не возвращайтесь
   к этому шагу:
   `http://localhost:8477/api/auth/google/callback` и
...
Обычный способ — одной командой. Она собирает образ, если исходники свежее его, поднимает
базу и приложение, накатывает миграции и открывает браузер:

```bash
ln -s "$PWD/bin/workflow" ~/.local/bin/workflow   # один раз
workflow                                          # http://localhost:$APP_PORT
```

| Команда | Что делает |
|---|---|
| `workflow` | Поднять всё и открыть стол в браузере |
| `workflow stop` | Остановить контейнеры, не удаляя их |
| `workflow restart` | Перезапустить |
| `workflow logs` | Хвост лога приложения |
| `workflow password` | Сменить пароль входа: спросит новый, впишет в `.env`, перезапустит |

Для разработки приложение поднимается мимо контейнера, с горячей перезагрузкой:

```bash
docker compose up -d db   # только Postgres
pnpm db:migrate           # в контейнерном запуске миграции накатываются сами
pnpm dev                  # http://localhost:3000
```

Дальше — войти паролем, который вы захешировали, и на `/settings` подключить
Google-аккаунты по OAuth. Каждый следующий аккаунт добавляется там же и получает свой
цвет; календарь внутри аккаунта может этот цвет перебить.
...
```bash
claude mcp add --transport http workflow http://localhost:8477/api/mcp \
  --header "Authorization: Bearer ваш-токен"
```

Порт здесь тот же, по которому вы открываете стол: `APP_PORT` или 3000 при `pnpm dev`.

Пока `MCP_BEARER_TOKEN` пуст, эндпоинта не существует — он отвечает 404, а не работает без
проверки. С неверным токеном — 401.

Правку `.env` контейнер подхватывает при пересоздании, а не при любом запуске: после
изменения переменных зовите `workflow restart`, а не `workflow`.
...
| `workflow` | Поднять всё в контейнерах и открыть браузер |
...
src/
  ...
  lib/
bin/
  workflow           запуск приложения одной командой
  mcp-stdio.ts       точка входа MCP для Claude Desktop
```
